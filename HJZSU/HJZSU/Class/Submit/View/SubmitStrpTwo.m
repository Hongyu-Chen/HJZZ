...
@interface SubmitStrpTwo ()<UITableViewDelegate,UITableViewDataSource>
...
- (void)setModel:(SubmitModel *)model{
    ...
    if (_model) {
        [self getPackageWith:_model.activityType success:^(id  _Nonnull result) {
            self.data = result;
            if (self.model.activityType == 4) {
                for (PlanModel *model in self.data) {
                    model.supervision_number = 0;
                }
            }
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView{
    return self.data ? self.data.count + 2 : 2;
}

- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    if (self.data) {
        if(section == self.data.count){
            return self.model.activityType == 5 ? 0 : 1;
        }
        else{
            return 2;
        }
    ...
    if (self.data) {
        if (indexPath.section < self.data.count) {
            if (indexPath.row == 0) {
                ...
                cell.model = self.data ? self.data[indexPath.section] : nil;
        ...
        else if (indexPath.section == self.data.count){
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (self.data) {
        if (indexPath.section == self.data.count + 1 && indexPath.row == 1){
                ...
                if (tmp.selected) {
                    ...
                    safe.safeBlock = ^(SafeModel * _Nonnull model, VocherModel * _Nonnull vocherModel) {
                        if (weakself.model.activityType == 5) {//培训价格计算方式
                            ...
                            weakself.model.allMoneny = weakself.model.plan.price_controller;
                        }
                        else{
                        ...
                        if (model) {
                            ...
                            weakself.model.allMoneny +=((model ? model.buy_money : 0) - (vocherModel ? vocherModel.worth_money/100 : 0));
                        }
                        else{
                            weakself.model.safe = nil;
                            weakself.model.vocherModel = nil;
